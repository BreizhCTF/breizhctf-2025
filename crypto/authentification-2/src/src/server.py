...
# Tu peux utiliser cette fonction pour reset la db
#       (et pouvoir re-créer des users)
# de manière plus rapide qu'en rédemarrant l'instance
@app.route('/reset-db')
def reset_db():
    global MASTER_KEY
    global users
...
def admin():
    ...
    try:
        if auth_cookie == None:
            msg = "Tu n'es pas connecté(e) :("
        elif verif_token(MASTER_KEY, auth_cookie) == True:
            msg = f"Félicitations ! Voici ton flag : {FLAG} :)"
        else:
            msg = "Tu n'es pas un super_admin :("
    except:
        msg = "Erreur avec le token..."
