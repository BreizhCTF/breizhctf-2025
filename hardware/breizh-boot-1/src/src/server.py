...
def handle_flash():
    data = get_input("Enter base64-encoded fitImage: ")
...
def format_disk():
    ...
    subprocess.run(
        ["dd", "if=/dev/zero", "of=upload/fitImage-disk.img", "bs=1M", "count=10"],
        check=True,
    )
    subprocess.run(["mformat", "-i", "upload/fitImage-disk.img", "::"], check=True)

    # Use FUSE-based mount with fusefat
    subprocess.run(
        ["mcopy", "-i", "upload/fitImage-disk.img", "upload/fitImage", "::"], check=True
    )

    print("Disk formatted and fitImage copied successfully.")
...
def boot_virt():
    subprocess.run(
        [
            ...
            "barebox-dt-2nd.img",
            "-drive",
            "file=upload/fitImage-disk.img,format=raw,media=disk,id=hd0",
            "-device",
            "virtio-blk-device,drive=hd0",
            "-drive",
            "file=flag.img,format=raw,media=disk,id=hd1",
            "-device",
