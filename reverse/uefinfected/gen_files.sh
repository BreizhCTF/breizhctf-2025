...
run_command docker build -t uefinfected -f ./src/Dockerfile.build ./src
...
run_command docker run --rm -d --name uefinfected uefinfected:latest
...
run_command docker cp uefinfected:/UEFInfected.efi ./files/
...
# Arrêt du conteneur
run_command docker stop uefinfected
...
run_command docker rmi uefinfected
